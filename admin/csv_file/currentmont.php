...
// Function to check if a day is in the future (including today)
function isFutureDay($day, $year, $month) {
    $currentDate = new DateTime("now", new DateTimeZone("asia/kolkata"));
    $checkDate = new DateTime("$year-$month-$day", new DateTimeZone("asia/kolkata"));
...
// Get yesterday's date (last day with complete data)
...
foreach ($employees as $employee) {
    ...
    for ($day = 1; $day <= $daysInMonth; $day++) {
        ...
        // Check if weekend
        if (isWeekend($day, $currentYear, $currentMonth)) {
            $row[] = "Holiday";
            $counters['holiday']++;
            continue;
        }
        ...
        $stmt->execute([$employee['emp_id'], $day, $currentMonth, $currentYear]);
        ...
        if (empty($attendance)) {
            ...
            foreach ($attendance as $record) {
                $dateStr = "$currentYear-$currentMonth-" . sprintf('%02d', $day);
    ...
    $row[] = array_sum([$counters['sick_leave'], $counters['casual_leave'], $counters['half_day'], $counters['regularization'], $counters['present'], $counters['wfh'], $counters['holiday']]); // Total working days
