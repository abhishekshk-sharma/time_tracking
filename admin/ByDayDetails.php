<?php
    ...
    // if(isset($_SESSION['ack'])){
        ...
        $join = new DateTime($emp_details['hire_date']);
        ...
        $rowdate = isset($_SESSION['searchDate'])?$_SESSION['searchDate']:'';
        if($rowdate != ''){
            $date = $rowdate;
        }
        ...
        <div class="main-content">
            ...
            <div class="content-area">
                ...
                <div class="action-buttons">
                    <button class="btn btn-primary" onclick="window.location.href='attnd_history.php'">
...
<script>
    ...
    // Notification section end
        ...
        function detailsById(){
            var info = "activityByDay";
            var id ='<?php echo $userID; ?>';
            var date = '<?php echo $date; ?>';
            $.ajax({
                url: "time_management/total.php",
                method: "POST",
                data: {info:info, id: id, date:date},
                success: function(e){
                    // alert(e);
                    if(e){
        ...
        $(document).on("click", ".activity-name", function(){
            var id = $(this).data("id");
            // alert(id);
            var info = "activityDetailByIdforeditmodal";
            $.ajax({
                url: "time_management/total.php",
                method: "POST",
                dataType: 'json',
                ...
                success: function(e){
                    ...
                    if(e){
                        ...
                        if(e.status == "fetched"){
                            ...
                            functionforeditentry(e.data.id, e.data.entry_type, e.data.entry_time);
                        }
                    }
        ...
        function functionforeditentry(id, entry_type, entry_time){
            // alert("function");
            Swal.fire({
                title: 'Edit Entry',
                html:
                    '<input id="swal-input1" class="swal2-input" value="'+entry_type+'" readonly style="cursor: not-allowed;">' +
                    '<input id="swal-input2" type="datetime-local" class="swal2-input" value="'+entry_time+'">' +
                    '<input id="swal-input3" type="hidden" class="swal2-input" value="'+id+'">',

                confirmButtonText: "Update",
                showCancelButton: true,
                preConfirm: () => {
                    const entry_type = document.getElementById('swal-input1').value;
                    const entry_time = document.getElementById('swal-input2').value;
                    const id = document.getElementById('swal-input3').value;
                    if (!entry_type || !entry_time) {
                        Swal.showValidationMessage('Please enter both entry type and time');
                   
                        return false;
                    }
                    return { entry_type: entry_type, entry_time: entry_time, id:id };
          
                }
            }).then((response)=>{
                if(response.isConfirmed){
                    var info = "updateEntryById";
                    var entry_type = response.value.entry_type;
                    var entry_time = response.value.entry_time;
                    var id = response.value.id;
                    $.ajax({
                        url: "time_management/total.php",
                        method: "POST",
                        dataType: 'json',
                        data: {info:info, entry_type:entry_type, entry_time:entry_time, id:id},
                        success: function(e){
                            if(e.status == "updated"){
                                ...
                                Swal.fire('Error', 'Failed to update entry'+e, 'error');
                                detailsById();
                            }
                        },
